...
class SnpiPhy:
	def __init__(self, threads, cutoff, reads_info, outdir, reference, prefix, model, tree_builder, single_end, no_recombination_filter):
		...
		self.initial_trees = snpiphy.define_make_outdirs(os.path.join(self.outdir, "initial_trees"))
		...
		self.init_tree = os.path.join(self.outdir, '.'.join((prefix,"initial.tree.nex")))
		self.init_filtered_tree = os.path.join(self.outdir, '.'.join((prefix,"initial_rfiltered_tree.nwk")))
		self.bootstrap_tree = os.path.join(self.outdir, '.'.join((prefix,"bootstap_trees.nex")))
		self.no_recombination_filter = no_recombination_filter
		if no_recombination_filter == True:
	...
	def remove_results_without_reads(self):
		...
		for x in os.listdir(self.ref_aligns):
			...
			if os.path.isdir(abspath) == True:
				for read_path in self.all_reads_list:
	...
	def Ns2gaps(self):
		if os.path.isfile(self.N_trimmed_core_aln) == False:
			...
			shutil.move(os.path.join(self.core_align, "core.trimmed.aln"), self.N_trimmed_core_aln)
			os.symlink(self.N_trimmed_core_aln, os.path.join(self.core_align, "core.trimmed.aln"))
		else:
	...
	def calculate_core_genome_stats(self):
		self.logger.info("Calculating core genome stats...")
		ref_seq_len = sum(len(x) for x in SeqIO.parse(self.reference, 'fasta'))
		core_aln = AlignIO.read(os.path.join(self.core_align, 'core.full.aln'), 'fasta')
		core_genome_size = 0
		for i, column in enumerate(core_aln[0]):
			if ('-' in core_aln[:, i]) or ('N' in core_aln[:, i]):
				continue
			else:
				core_genome_size += 1
		self.logger.info("Core genome size = {} ({:.2f}%% of reference)".format(core_genome_size, (100*float(core_genome_size)/ref_seq_len)))
	...
	def count_core_snps(self):
		if os.path.isfile(self.core_snp_stats) == False:
			if self.no_recombination_filter == True:
				...
				snpiphy.count_snps(os.path.join(self.core_align, 'core.trimmed.aln'), self.core_snp_matrix, self.core_snp_stats, self.threads)
				self.calculate_core_genome_stats()
			else:
				self.logger.info("Calculating pairwise recombination-filtered_core SNPs...")
				...
				self.calculate_core_genome_stats()
				self.logger.info("Note core reference size is calculated on the non recombination filtered genome alignment...".format(core_genome_size, (float(core_genome_size)/ref_seq_len)))
		else:
